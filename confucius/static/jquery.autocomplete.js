...
jQuery.autocomplete = function(input, options) {
		...
		switch(e.keyCode) {
			...
			case 13: // return
				...
				for (var i=0, j=options.data.length; i<j; i++){
					sFirstChar = options.data[i][0].toLowerCase();
					if (options.data[i]==$input.get(0).value){
						subFromCache(sFirstChar, options.data[i]);
	...
	function selectItem(li) {
		...
		$input.val(v);
		hideResultsNow();
		if (options.onItemSelect) setTimeout(function() { options. (li) }, 1);
	};
...
function subFromCache(q, data){
	for (var i=0, j=cache.data[q].length; i<j; i++){
		if ($.trim(cache.data[q][i])==data){
			cache.data[q].splice(i, 1);
			break;			
		}					
	}		
}
...
//TO DO
function addDataToCache(data){
		// if there is a data array supplied
		if( options.data != null ){
			var sFirstChar = data.substring(0, 1).toLowerCase(), stMatchSets = {}, row = [];
